...
### CORS on the bucket (required for E2E)

The non-E2E download path is a 302 from the backend to a presigned S3
URL. Browsers follow 302s as top-level navigations, which aren't
subject to CORS — so non-E2E deployments don't need any CORS config.

The E2E path is different. The recipient's Service Worker
...
the body — used by the E2E Service Worker so it can fetch S3 with a
