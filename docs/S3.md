...
### CORS on the bucket (required for encryption)

The non-encrypted download path is a 302 from the backend to a presigned S3
URL. Browsers follow 302s as top-level navigations, which aren't
subject to CORS — so non-encrypted deployments don't need any CORS config.

The encryption path is different. The recipient's Service Worker
(`frontend/public/sw.js`) calls
`GET /api/v1.0/downloads/<token>/files/<id>/download/?as=json`, gets
back `{"url": "<presigned>"}`, and then `fetch()`es the presigned URL
directly to stream-decrypt the ciphertext. That second fetch is
cross-origin from the frontend to the S3 origin — the bucket must
return `Access-Control-Allow-Origin` and friends, or the fetch fails
in the SW and the recipient sees a download error.

Minimum config — adjust `AllowedOrigin` to your deployment:

```xml
<CORSConfiguration>
  <CORSRule>
    <AllowedOrigin>https://transferts.example.gouv.fr</AllowedOrigin>
...
In dev, RustFS has `RUSTFS_CORS_ALLOWED_ORIGINS=*` set in
`compose.yaml`, so the SW path "just works" without further config.

### `?as=json` on the download endpoint

The download view returns a 302 by default. Adding `?as=json` to the
URL flips it to a 200 response with `{"url": "<presigned S3 URL>"}` in
the body — used by the encryption Service Worker so it can fetch S3 with a
fresh anonymous request instead of following a cross-origin redirect
with credentials (which Firefox NS_ERROR_FAILUREs on). The
`FILE_DOWNLOADED` audit event fires identically on both branches.
